...
CShootingGun::CShootingGun(LPDIRECT3DDEVICE9 pGraphicDev)
...
CShootingGun::~CShootingGun()
{
	Release();
}

int CShootingGun::Update()
{
	CUiPlate::Update();
...
void CShootingGun::Render()
{
	...
	m_pGraphicDev->SetRenderState(D3DRS_ALPHAREF, 80); // 알파 기준 값 1로 설정.
...
HRESULT CShootingGun::Initialize()
{
	CUiPlate::Initialize();
	return S_OK;
}

HRESULT CShootingGun::AddComponent()
{
	CUiPlate::AddComponent();
	return S_OK;
}

void CShootingGun::Release()
{
	CUiPlate::Release();
}

void CShootingGun::Render_Count()
{
	ZeroMemory(&m_szTextureCount, sizeof(m_szTextureCount));
	swprintf_s(m_szTextureCount, L"%d", m_iCount);
}

void CShootingGun::Notify_Count(int iCount)
{
	m_iCount = iCount;
}

CShootingGun * CShootingGun::Create(LPDIRECT3DDEVICE9 pGraphicDev, D3DXVECTOR3 vPos, D3DXVECTOR3 vSize, wstring wstrTextureTag, int iIndex)
{
	NULL_CHECK_RETURN(pGraphicDev, nullptr);

	CShootingGun* pInstance = new CShootingGun(pGraphicDev);
