...
CCottenCandyShop::CCottenCandyShop(LPDIRECT3DDEVICE9 pGraphicDev)
...
CCottenCandyShop::~CCottenCandyShop()
{
	Release();
}

int CCottenCandyShop::Update()
{
	ENGINE::CGameObject::Update();
	return NO_EVENT;
}

void CCottenCandyShop::LateUpdate()
{
	ENGINE::CGameObject::LateUpdate();
}

void CCottenCandyShop::Render()
{
	...
	//dynamic_cast<ENGINE::CVIBuffer*>(m_mapComponent[L"Buffer_CottenCandyShop"])->SetConvertVtx(m_pConvertVtx);


	m_mapComponent[L"Texture"]->Render(0);

	m_mapComponent[L"Buffer_CottenCandyShop"]->Render();
}

HRESULT CCottenCandyShop::Initialize()
{
	FAILED_CHECK_RETURN(AddComponent(), E_FAIL);

	
	ENGINE::CVIBuffer* pBuffer = dynamic_cast<ENGINE::CVIBuffer*>(m_mapComponent[L"Buffer_CottenCandyShop"]);
...
HRESULT CCottenCandyShop::AddComponent()
{
	ENGINE::CComponent* pComponent = nullptr;

	// Buffer
	pComponent = m_pResourceMgr->CloneResources(ENGINE::RESOURCE_DYNAMIC, L"Buffer_CottenCandyShop");
	NULL_CHECK_MSG_RETURN(pComponent, L"Buffer_CottenCandyShop Clone Failed", E_FAIL);
	m_mapComponent.insert(make_pair(L"Buffer_CottenCandyShop", pComponent));


	// Texture
	pComponent = m_pResourceMgr->CloneResources(ENGINE::RESOURCE_DYNAMIC, L"Texture_CottenCandyShop");
	NULL_CHECK_MSG_RETURN(pComponent, L"Texture_CottenCandyShop Clone Failed", E_FAIL);
...
CCottenCandyShop * CCottenCandyShop::Create(LPDIRECT3DDEVICE9 pGraphicDev, D3DXVECTOR3 vPos, D3DXVECTOR3 vSize)
{
	NULL_CHECK_RETURN(pGraphicDev, nullptr);

	CCottenCandyShop* pInstance = new CCottenCandyShop(pGraphicDev);
