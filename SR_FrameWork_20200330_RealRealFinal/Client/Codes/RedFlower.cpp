...
CRedFlower::CRedFlower(LPDIRECT3DDEVICE9 pGraphicDev)
...
CRedFlower::~CRedFlower()
{
	Release();
}

int CRedFlower::Update()
{
...
void CRedFlower::LateUpdate()
{
	ENGINE::CGameObject::LateUpdate();
	IsBillboarding();
}

void CRedFlower::Render()
{
...
HRESULT CRedFlower::AddComponent()
{
	ENGINE::CComponent* pComponent = nullptr;

	// Buffer
	pComponent = m_pResourceMgr->CloneResources(ENGINE::RESOURCE_DYNAMIC, L"Buffer_RedFlower");
	NULL_CHECK_MSG_RETURN(pComponent, L"Buffer_RedFlower Clone Failed", E_FAIL);
	m_mapComponent.insert(make_pair(L"Buffer", pComponent));

	// Texture
	pComponent = m_pResourceMgr->CloneResources(ENGINE::RESOURCE_DYNAMIC, L"Texture_RedFlower");
	NULL_CHECK_MSG_RETURN(pComponent, L"Texture_RedFlower Clone Failed", E_FAIL);
...
void CRedFlower::IsBillboarding()
{
	CCameraObserver* pObserver = dynamic_cast<CCameraObserver*>(m_mapComponent[L"Observer"]);
	NULL_CHECK(pObserver);
...
CRedFlower* CRedFlower::Create(LPDIRECT3DDEVICE9 pGraphicDev, D3DXVECTOR3 vPos, D3DXVECTOR3 vSize)
{
	NULL_CHECK_RETURN(pGraphicDev, nullptr);

	CRedFlower* pInstance = new CRedFlower(pGraphicDev);
