...
HRESULT CCrossHair::AddComponent()
{
	CUiPlate::AddComponent();
	return S_OK;
}

void CCrossHair::Release()
{
	CUiPlate::Release();
}

void CCrossHair::Render_Count()
{
	ZeroMemory(&m_szTextureCount, sizeof(m_szTextureCount));
	swprintf_s(m_szTextureCount, L"%d", m_iCount);
}

void CCrossHair::Notify_Count(int iCount)
{
	m_iCount = iCount;
}

CCrossHair * CCrossHair::Create(LPDIRECT3DDEVICE9 pGraphicDev, D3DXVECTOR3 vPos, D3DXVECTOR3 vSize, wstring wstrTextureTag, int iIndex)
{
