...
CUiMoleStart::CUiMoleStart(LPDIRECT3DDEVICE9 pGraphicDev)
	: CUiPlate(pGraphicDev)
{
}


CUiMoleStart::~CUiMoleStart()
{
	Release();
}

int CUiMoleStart::Update()
{
	CUiPlate::Update();
...
HRESULT CUiMoleStart::Initialize()
{
	CUiPlate::Initialize();
	return S_OK;
}

HRESULT CUiMoleStart::AddComponent()
{
	CUiPlate::AddComponent();
	return S_OK;
}

void CUiMoleStart::Release()
{
	CUiPlate::Release();
}


CUiMoleStart * CUiMoleStart::Create(LPDIRECT3DDEVICE9 pGraphicDev, D3DXVECTOR3 vPos, D3DXVECTOR3 vSize, wstring wstrTextureTag, int iIndex)
{
	NULL_CHECK_RETURN(pGraphicDev, nullptr);

	CUiMoleStart* pInstance = new CUiMoleStart(pGraphicDev);
